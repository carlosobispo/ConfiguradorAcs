...
public final class DatedFileAppender extends FileAppender {
	...
	public void append(LoggingEvent event) {
		...
		if (n >= mTomorrow) {
			...
			String datestamp = datestamp(mCalendar); // convert to string
														// "yyyy-mm-dd"
			...
			mTomorrow = mCalendar.getTime().getTime(); // time in milliseconds
	...
	public static String datestamp(Calendar calendar) {
		...
		if (year < Parameters.MIL) {
			sb.append('0');
			if (year < Parameters.CIEN) {
				sb.append('0');
				if (year < Parameters.DIEZ) {
					sb.append('0');
				}
			}
		}
		...
		sb.append('-');
		if (month < Parameters.DIEZ) {
			sb.append('0');
		}
		sb.append(Integer.toString(month));
		sb.append('-');
		if (day < Parameters.DIEZ) {
			sb.append('0');
		}
