...
public class VAcsUserDAO implements AcsUserInterface{
	...
	public List<AcsUserBean> listaAcsUser() throws Exception {
		...
		try {
			Utilitario.getLOG_SQL().info("START: VAcsUserDAO - listaAcsUser");
			...
			listUrl = query.list();
			Utilitario.getLOG_SQL().info("FINISH: VAcsUserDAO - listaAcsUser");
		
		} catch (Exception e) {
			Utilitario.getLOG_SQL().error("ERROR: VAcsUserDAO - listaAcsUser Hubo un problema al momento de obtener la informacion.",e);
		}
	...
	public List<AcsUserBean> listAcsUserByIdIssuer(String idIssuer) throws Exception {
		List<AcsUserBean> listUrl= null;
		try {
			Utilitario.getLOG_SQL().info("START: VAcsUserDAO - listAcsUserByIdIssuer()");
			iniciarOperacion();
			...
			listUrl = query.list();
			Utilitario.getLOG_SQL().info("FINISH: VAcsUserDAO - listAcsUserByIdIssuer()");
		} catch (Exception e) {
			Utilitario.getLOG_SQL().error("ERROR: VAcsUserDAO - listAcsUserByIdIssuer() Hubo un problema al momento de obtener la informacion.",e);
			Utilitario.getLOG_SQL().error("ERROR: VAcsUserDAO - listAcsUserByIdIssuer()", e.getCause());
		}
	...
	public AcsUserBean getAcsUserById(Integer idAcsUser) throws Exception {
		...
		try {
			Utilitario.getLOG_SQL().info("START: VAcsUserDAO - getAcsUserById");
			iniciarOperacion();
			objAcsUser= (AcsUserBean) sesion.get(AcsUserBean.class, idAcsUser);	
					
			Utilitario.getLOG_SQL().info("FINISH: VAcsUserDAO - getAcsUserById");
		}catch (Exception e) {
			Utilitario.getLOG_SQL().error("ERROR: VAcsUserDAO - getAcsUserById() Hubo un problema al momento de obtener CardRange: "+idAcsUser, e);
		}
	...
	public boolean registrarAcsUser(AcsUserBean acsUser) throws Exception {
		...
		try {
			Utilitario.getLOG_SQL().info( "START: VAcsUserDAO - registrarAcsUser()");
			iniciarOperacion();
			Utilitario.getLOG_SQL().info( "Datos a registrar : " + acsUser.toString());
			sesion.save(acsUser);
			isValidoRegistro = true;
			Utilitario.getLOG_SQL().info("FINISH: VAcsUserDAO - registrarAcsUser()");
		} catch (Exception e) {
			Utilitario.getLOG_SQL().error("ERROR: VAcsUserDAO - registrarAcsUser() Hubo un problema al momento de registrar url ", e);
			manejaExcepcion();
		}
	...
	public boolean actualizarAcsUser(AcsUserBeanView acsUserView) throws Exception {
		...
		try {
			Utilitario.getLOG_SQL().info("STARD: VAcsUserDAO - actualizarAcsUser()");
			iniciarOperacion();
...
//			Utilitario.getLOG_SQL().info("Datos a actualizar : "+emisor.toString());
			...
			//transaction.commit();
			isValidoRegistro=true;
			Utilitario.getLOG_SQL().info("FINISH: VAcsUserDAO - actualizarAcsUser()");
		} catch (Exception e) {
			Utilitario.getLOG_SQL().error("ERROR: VAcsUserDAO - actualizarAcsUser() Hubo un problema al momento de actualizar actualizarAcsUser ",e);
			manejaExcepcion();
		}
